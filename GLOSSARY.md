...
**In this project:** `extract_azure_to_snowflake.py` is idempotent by design — re-running for the same date window `DELETE`s the target slice before re-loading, so a mid-run crash + restart produces identical output to a clean single run. `ENGINEERING_STANDARDS.md` lists idempotency as failsafe #8.
...
**In this project:** `ENGINEERING_STANDARDS.md` criterion 7 is "Upstream/downstream contract." `extract_azure_to_snowflake.py` verifies source vs destination row counts on every run.
...
### **Multi-Active Satellite (MAS)** `[Project 3]`

A Data Vault 2.0 satellite that allows MULTIPLE concurrent active rows per parent hash key at the same point in time. Relaxes the 1:1-parent-coverage invariant of standard satellites. The textbook example is a customer with multiple active phone numbers simultaneously — a standard sat would force one row per customer per load_datetime and reject the others as PK violations. The MAS resolves this by extending the natural primary key from `(parent_hk, load_datetime)` to `(parent_hk, child_dependent_key, load_datetime)` — adding a third column (the [sub-sequence key / CDK](#sub-sequence-key-child-dependent-key-cdk)) that identifies which active row this is. Standard SCD-2 change-detection mechanics still apply, but the anti-join filter must partition by `(parent_hk, sub_sequence_key)` rather than parent_hk alone, otherwise newly-extracted rows would compare against the wrong active row's stored hashdiff. Verified against AutomateDV's ma_sat tutorial and Scalefree's multi-active-satellites Part 1 article.

**In this project:** `sat_concept_canonical` — first MAS in the project, shipped Phase 2 session 9. Parent = `hub_concept` (5 canonical concepts); 8 active rows total (canonical `'revenue'` has 4 concurrent active raw XBRL US-GAAP tag names — Revenues / SalesRevenueNet / Revenue...ExcludingAssessedTax / Revenue...IncludingAssessedTax — that all map to the same canonical via the seed dictionary; the other 4 canonicals are identity-mapped 1:1). Single-column surrogate `sat_concept_canonical_hk` extends the sessions 6/7/8 two-component sat hash to three components by adding `sub_sequence_key` between parent_hk and load_datetime. The MAS has a "degenerate payload" — the raw concept_name IS both the active-row identifier (hashed into the CDK) and the only descriptive attribute being preserved, so the hashdiff column is structurally constant per (parent, CDK) pair by construction (LEARNINGS Risk 17, 2026-05-29). Locked at LEARNINGS Risks 17 + 18, 2026-05-29. See `DBT_PIPELINE.md` section 8.20 for the full architectural walkthrough.

### **Sub-sequence key / Child Dependent Key (CDK)** `[Project 3]`

The column added to a [multi-active satellite](#multi-active-satellite-mas)'s natural primary key to identify which of the multiple concurrent active rows for a given parent this row is. Two construction options exist in the DV2.0 literature, with Scalefree's multi-active-satellites Part 1 explicitly prioritising one over the other. **Preferred:** use a stable source-provided type code (e.g., phone type 'home'/'business'/'cell' on a customer-phone MAS) hashed to fixed width for surface consistency with the rest of the hash-key column set. **Fallback:** if no stable type code is provided, generate a sub-sequence number per parent in the staging layer and use that as the CDK. Auto-numbering is the fallback rather than the default because re-shuffling the sub-sequence assignment across loads would corrupt the audit lineage — a row's `sub_sequence_key = 3` on Monday could become `sub_sequence_key = 2` on Tuesday if the upstream source returned the active rows in different order, breaking SCD-2 history.

**In this project:** `sat_concept_canonical.sub_sequence_key` = `to_hex(sha256(to_utf8(CAST(concept_name AS varchar))))` — SHA-256 of the raw XBRL US-GAAP tag name as the CDK. Raw tag names are stable source-provided taxonomy identifiers (a 10-K filed under `'Revenues'` in 2017 is still tagged `'Revenues'` on every re-extract today), satisfying Scalefree's "stable type code" criterion. Auto-numbering rejected as fragile under seed reordering. The CDK construction parallels the rest of the project's hash-chain pattern (same SHA-256 + to_utf8 + to_hex chain as every other hash column), giving the warehouse-layer surface visual consistency: every hash column is 64 hex chars wide. Locked at LEARNINGS Risk 18, 2026-05-29. See `DBT_PIPELINE.md` section 8.20.

### **Business Vault** `[Project 3]`

The Scalefree-canonical Data Vault 2.0 layer between the [Raw Vault](#data-vault-2-0-the-second-major-modelling-pattern-project-3) (hubs, links, satellites — source-of-truth audit-lineage records) and downstream information marts. Business Vault objects are **derived** — every row is computed from Raw Vault content via deterministic transformations, never from new source data. The layer exists to flatten join graphs, materialize grain shifts, and pre-compute query helpers that downstream analytical queries would otherwise repeat at runtime. Two canonical object classes live here: [PIT (Point-In-Time) tables](#point-in-time-pit-table-project-3) and [Bridge tables](#bridge-table-project-3). Because BV objects are derived rather than sourced, the audit-lineage contract that governs Raw Vault (immutable rows, never updated) doesn't apply — BV tables are rebuilt every refresh cycle from the current Raw Vault state. The deliberate separation between Raw Vault (source of truth, append-only) and Business Vault (derived, refreshable) is what lets DV2.0 implementations evolve mart-side query optimizations without re-engineering the audit-lineage layer.

**In this project:** Business Vault shipped Phase 2 session 10. Three models in `dbt/models/business_vault/`: `dim_as_of_dates` (10-row as-of-dates spine, fiscal year-ends 2016-2025), `pit_link_filing_concept_period` (single-sat PIT on the project's most-queried link spine, 634,431 rows), `bridge_company_concept_period` (5-hop hub-link-hub navigation Bridge, 634,431 rows). Materialized as plain Iceberg tables (not incremental merge) per `business_vault` layer defaults in `dbt_project.yml` — full rebuild each dbt run is the BV pattern and structurally avoids the Iceberg-merge `on_schema_change` duplicate-insertion bug class (LEARNINGS Risk 2). See `DBT_PIPELINE.md` sections 8.22-8.25 for the full architectural walkthrough.

### **Point-in-Time (PIT) table** `[Project 3]`

A [Business Vault](#business-vault-project-3) object that pre-computes, for each (parent hash key, as-of-date) pair, the coordinates (hash key + load_datetime) of the relevant satellite row at that as-of-date. Replaces the SCD-2 latest-row anti-join every downstream query would otherwise repeat at runtime with a single equi-join lookup. Especially valuable when a parent has 2+ satellites with different update rates — the PIT collapses what would be N satellite-anti-join scans into one row-fetch per parent per as-of-date. Per Scalefree's PIT structure article + AutomateDV's PIT tutorial, the canonical structure is `(parent_hk, as_of_date, [sat_n_pk, sat_n_ldts] for each resolved sat)`. PITs are rebuilt every refresh — they're not historized.

**In this project:** `pit_link_filing_concept_period` — single-sat PIT shipped Phase 2 session 10. Spine = `link_filing_concept_period` (89,821-row 3-way standard link), resolved sat = `sat_concept_value`. 10 as-of-dates from `dim_as_of_dates` (fiscal year-ends 2016-2025) × link rows visible at each as-of-date = 634,431 rows. Picked this spine over hub-level PITs because `link_filing_concept_period` is THE fact-shape every Phase 4 mart consumes; single-sat PIT framing acknowledged explicitly (LEARNINGS Risk 19, 2026-05-29) as demonstrative-of-pattern rather than padding-the-warehouse-with-PITs. Temporal anchor = `filed_date` from `sat_filing_metadata` instead of canonical `load_datetime` per LEARNINGS Risk 23 (2026-05-29 — project's load_datetime captures ingestion-time, not observation-time). Ghost-record pattern deferred (LEARNINGS Risk 22) — LEFT JOIN + NULL substitutes inner-join-on-zero-key semantics. See `DBT_PIPELINE.md` section 8.23.

### **Bridge table** `[Project 3]`

A [Business Vault](#business-vault-project-3) object that pre-computes hub-link-hub navigation paths at a given as-of-date. Replaces multi-link join walks with a single table scan. Per Scalefree's Bridge Tables 101 + AutomateDV's bridge tutorial, the canonical structure is a "specialised form of link" carrying hash keys from the parent hub plus the links it spans, plus an as-of-date dimension, plus optional aggregated measures (for grain-shift scenarios where the bridge's grain is coarser than the underlying link's). Bridges are rebuilt every refresh. AutomateDV's textbook structure assumes each link carries an Effectivity Satellite tracking relationship end-dates; Scalefree's simpler shape omits eff_sat columns when source relationships are insert-only-current.

**In this project:** `bridge_company_concept_period` — first Bridge in the project, shipped Phase 2 session 10. Spine = `hub_company`. Walks the 5-hop graph hub_company → link_company_filing → hub_filing → link_filing_concept_period → hub_concept. Each row carries the 3 hub hash-key FKs + 2 link hash-key FKs + 3 link-level period payload columns (period_end_date, fiscal_year, fiscal_period) + as_of_date + lineage. 634,431 rows = identical count to the sister PIT by construction (both share the same link × as_of_date × filed_date visibility filter). No effectivity satellites — our links are insert-only with no relationship-end-date semantics in SEC reporting (LEARNINGS Risk 20, 2026-05-29). Temporal anchor = `filed_date` per Risk 23. Phase 4 `mart_pl_trend` and `mart_peer_benchmark` will equi-join this Bridge directly instead of walking the Raw Vault — single SELECT replaces the 5-table JOIN graph. See `DBT_PIPELINE.md` section 8.24.

### **As-of-date table** `[Project 3]`

A small reference table holding the list of timestamps for which [PIT](#point-in-time-pit-table-project-3) and [Bridge](#bridge-table-project-3) objects pre-compute "what was visible at this date" snapshots. Required input for both PIT and Bridge build logic (per AutomateDV's `as_of_dates_table` parameter + Scalefree's as-of-date discussion). Cardinality choice directly multiplies PIT/Bridge row counts: 10 yearly snapshots over a 10-year history × an 89K-row link spine = 898K-row theoretical-max PIT. Senior-DE design call at every BV implementation: pick the as-of-date grain that matches downstream consumption (annual mart queries → yearly snapshots; quarterly → quarterly; daily → daily) rather than defaulting to a fine-grained aesthetic.

**In this project:** `dim_as_of_dates` — 10 rows = fiscal year-end dates 2016-12-31 through 2025-12-31. Picked yearly over quarterly (~38 rows → 3.4M PIT) per LEARNINGS Risk 21 (2026-05-29) to balance demonstrative multi-snapshot mechanic against storage budget. Materialized via a `SELECT * FROM (VALUES ...)` chain rather than a CSV seed for reproducibility-from-source. Columns: `as_of_date` (DATE, business key), `as_of_datetime` (timestamp upcast for sat-join compatibility), `fiscal_year_end` (convenience integer for mart-side grouping), `load_datetime`, `record_source`. See `DBT_PIPELINE.md` section 8.22.
...
Two ODBC connection-string parameters that together mean "TLS in transit AND validate the server's certificate against a trusted CA." Defaults vary by driver version — making them explicit is `ENGINEERING_STANDARDS.md` criterion 4.
...
**In this project:** Standard throughout Python scripts. `ENGINEERING_STANDARDS.md` criterion 1 (Currency) calls out `pathlib` over `os.path`.
...
Phil's project-specific code-review checklist (`ENGINEERING_STANDARDS.md`) — seven core checks (currency, compactness, resource efficiency, privacy/security, workflow consistency, dev environment hygiene, upstream/downstream contract) plus three failsafes (idempotency, pre-flight/post-action verification, observable progress and actionable errors). Applied to every non-trivial script before "done."
