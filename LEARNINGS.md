...
#### Risk 66 — bridge_fy `fiscal_period = 'FY'` filter rejects sat rows where SEC EDGAR companyfacts JSON omits the fp attribute on legitimate annual facts; mart-side period-end re-anchor cannot reach them (banked 2026-06-01, Phase 5 session 4.5 Step M re-audit)

**Discovered during Phase 5 session 4.5 Step M re-audit A4.2 sat-layer probe.** The Phase 5 session 4 Fix-all phase claimed 142 of 191 NULL cells would heal via Risk 58 period-end re-anchor. Step M re-audit measured 92 healed (43-cell shortfall). Drilldown on the 8-CIK net_income gap (BKNG / PNC / MA / TMO / CAT / CCI / AMT / SO at FY2024) showed BKNG / CAT / MA carrying their FY annual NetIncomeLoss rows in sat with `fiscal_period IS NULL` — SEC EDGAR's companyfacts JSON occasionally omits the `fp` attribute on top-level annual period_json entries for certain accession styles. The bridge_fy CTE filter `WHERE b.fiscal_period = 'FY'` rejected those rows BEFORE the Risk 58 mart-side period-end re-anchor could land them at year(period_end_date).

**Root cause.** Two filter layers, opposite design assumptions. The bridge_fy filter assumes SEC consistently populates fp on annual facts. The Risk 58 mart-side re-anchor assumes the row already exists in bridge. The latter's "heal" is conditional on the former passing. Risk 58 fixed the wrong layer of the chain for these CIKs.

**Triage / fix shape.** bridge_fy filter relaxed to `WHERE b.fiscal_period = 'FY' OR b.fiscal_period IS NULL` in all 3 marts (mart_financial_health, mart_pl_trend, mart_peer_benchmark) in lockstep. Quarterly Q1/Q2/Q3 rows in SEC data carry populated `fp` per convention, so the NULL branch only admits legitimate annual rows; the downstream Risk 48 span filter (350-380 day span) at sat_resolved is defense-in-depth that rejects any quarterly cumulative that might leak. Post-amendment cascade: BKNG / CAT / MA net_income cells healed; gross_profit / SE derivation chains also benefited where their derivation-input tags had fp=NULL annuals.

**Carry-forward.** When two filter layers in a chain make opposite assumptions about source-data shape (one filter requires X populated, downstream re-anchors on derived(Y)), the upstream "pre-filter" silently nullifies the downstream "heal." Pattern: any time a fix re-anchors on a column at layer N, audit the filter predicates at layer N-1, N-2 for whether they assume the original anchor column is populated. Specifically for SEC EDGAR companyfacts pipelines: NEVER strict-equality-filter on fp / fy / fiscal_period attributes — they're optional in the source JSON. Use span-based or canonical-aware filters at the sat layer.

---

#### Risk 67 — bridge `filed_date <= as_of_date` temporal visibility gate hides FY+N 10-K comparatives filed AFTER the latest as_of_date; SPGI + 5 calendar-Dec-filer FY2024 absences are this mechanism (banked 2026-06-01, Phase 5 session 4.5 Step M re-audit)

**Discovered during Phase 5 session 4.5 Step M re-audit A4.3 link-trace probe.** SO FY2024 net_income existed in sat at period_end=2024-12-31 / fiscal_period=FY / fiscal_year=2025 / accession=0000092122-26-000006 / value=$4.401B (a fy=2025 10-K comparative for FY2024). Bridge LEFT JOIN at the latest as_of_date (2025-12-31) returned NULL — no bridge row materialized for this link_hk at that snapshot. Root cause: bridge `filed_date <= as_of_date` filter — SO's FY2025 10-K filed in Q1 2026 has filed_date > 2025-12-31, so the link's filing is invisible at every snapshot ≤ 2025-12-31. Same mechanism as SPGI (no standalone FY2024 10-K filed; data only in FY2025 10-K filed 2026) but Step M extended the diagnosis from SPGI alone to 5 additional CIKs (SO / TMO / CCI / PNC / AMT).

**Root cause.** `dim_as_of_dates` capped at 2025-12-31. Real-world data continues to be filed after that horizon. Any FY+N 10-K (and its FY+N-1 comparative) filed after the cap becomes structurally invisible to every PIT / Bridge snapshot in the system. The fix requires extending the horizon, not editing the filter (the temporal visibility filter is correct by design).

**Triage / fix shape.** Extended `dim_as_of_dates` from 10 rows (2016-12-31 → 2025-12-31) to 11 rows by adding 2026-06-01 — a forward-snapshot capturing every CIK's most-recent filing as of the present "current" horizon. PIT / Bridge scan cost grows ~10% (well within the cardinality lock's envelope per Risk 21). Post-amendment cascade: SPGI FY2024 row present in mart_financial_health; SO / TMO / CCI net_income healed; PNC + AMT remained NULL via a different mechanism (their NetIncomeLoss undimensional .units.USD array is structurally empty for FY annual — documented as defended NULL).

**Carry-forward.** `dim_as_of_dates` is not a static configuration — it's the temporal envelope of the warehouse. Every time the calendar year rolls over OR a major filing window opens (Q1 10-K season in March / April), the cap must be re-evaluated. Pattern for any PIT-anchored warehouse: include a forward "current snapshot" date that's refreshed every dbt run, plus the historical year-end snapshots. For this project specifically: extend the forward snapshot on every cascade so newly-filed restatements become visible to subsequent analyses without a model edit.

---

#### Risk 68 — Glue Python Shell dbt build invocation must inherit local cascade's `--threads 2` flag; missing flag triggers Risk 64 S3 throttle in production despite local layer passing (banked 2026-06-01, Phase 5 session 4.5 Layer 3 Step Functions failure)

**Discovered during Phase 5 session 4.5 Layer 3 Step Functions production redrive.** Step M three-layer sign-off: Layer 1 local cascade (`dbt build --full-refresh --threads 2`) returned PASS=265 / ERROR=0 / SKIP=0; Layer 2 Athena spot-checks (sql/verify/18) returned 38/38 PASS. Layer 3 Step Functions execution failed at `RunDbtBuildOnGlue` with `Done. PASS=225 / ERROR=1 / SKIP=39`. CloudWatch log inspection showed `An error occurred (SlowDown) when calling the DeleteObjects operation (reached max retries: 5): Please reduce your request rate.` — the same Risk 64 S3 DeleteObjects throttle that the Fix-all phase already mitigated at the local layer.

**Root cause.** Risk 64's `--threads 2` mitigation was applied to local `dotenv -f ..\.env run -- dbt build --full-refresh --threads 2` invocations but `scripts/run_dbt_in_glue.py` (the Glue Python Shell entrypoint) never received the flag. dbt-athena's default 4-thread concurrency at the production layer continued busting the S3 burst limit. The "Step Functions production sign-off 102/102 green" closing line at Phase 5 session 4 hid this because that earlier session's production run was NOT a full-refresh (incremental sat rebuilds don't touch DeleteObjects at the same rate). Step M session 4.5 was the first full-refresh-equivalent rebuild in production after the Risk 64 mitigation, and surfaced the gap.

**Triage / fix shape.** Added `--threads 2` to the dbt build invocation in `scripts/run_dbt_in_glue.py`. Glue layer now lockstep with local layer's standing cascade command. Step Functions redrive in next session expected to clear.

**Closure (Phase 5 session 4.5 closeout, 2026-06-01).** Step Functions execution `phase5-s4-5-closeout-threads2-redrive-2026-06-01-v2` SUCCEEDED in 15:52, all 19 state transitions green (RunDbtBuildOnGlue + VerifyHubCompanyRowCount + 14-branch VerifyStructuralSurface Parallel block all TaskSucceeded). sql/verify/18 re-paste returned 48/48 PASS on the post-redrive warehouse. One debug step on the way: the first redrive attempt failed at deploy because `scripts/sync_phase3_artifacts_to_s3.py` had not been run since commit 7b17091 — S3 was still serving the OLD Glue script without `--threads 2`. Sync executed (49 dbt project files + 1 Glue script uploaded); deploy verified via boto3 get_object on the S3 script confirming `--threads` flag + `"2"` literal present; redrive then cleared first pass. Risk fully retired.

**Carry-forward.** When a project-wide mitigation lands as a "standing cascade command" (here: `--threads 2`), every layer that invokes the same tool must apply the same flag. Local dev / CI / production / Step Functions / Glue / Airflow / Dagster — each is a separate invocation path. Pattern: encode the flag as a project config (`dbt_project.yml threads:` or seed) rather than as a per-command CLI argument so all layers inherit by default. For this project specifically, follow-up Phase 6 polish: move `threads: 2` into the `dbt_project.yml` `+threads` config on the marts layer so the flag is automatic regardless of invocation path. Additional carry-forward banked Phase 5 session 4.5 closeout: every code-vs-S3 deploy gap — when local code is edited but the S3 artifact serving production isn't re-synced — should be caught BEFORE the production run is started, not via a 15-min Step Functions execution. Pre-run deploy verification (boto3 get_object + grep for the expected flag) should be the default check for any code change targeting a managed-runtime invocation surface.
...
**Going in.** The plan was the documented 2-4 hour fix: expand the
canonical revenue tag-mapping (canonical_concepts_dictionary +
canonical_concept_tag_preference seeds) plus the 6 lock-step Jinja
`{% set concepts %}` lists (int_sec_edgar__concepts + 5 warehouse
models), cascade-rebuild, re-verify. Per the standing "dbt mart is canon
— verify against data, not specs" + Risk 13 "empirical probe over
inferred parity" locks, session 2 opened with a diagnostic BEFORE
touching a single seed.

**Diagnostic (single consolidated Athena query, 2026-06-05).** Per-FY
revenue coverage at the latest snapshot + a per-(year, company) gap list
with each gap company's actual us-gaap revenue-family tags. Result:

- FY2022 / 2023 / 2024 / 2025 all at **100%** universe revenue coverage
  (107/107). FY2025 universe revenue = **$9.91T** — vs the Risk 55
  banking-day figure of $8.88T and the estimated true ~$10T. The
  10-12% sector-concentrated undercount is gone.
- The only low-coverage rows were pre-2016 sparse-XBRL history (expected)
  and the in-progress FY2026 calendar year (only the 7 Jan/Feb-FYE
  filers have filed — not a defect).
- The lone recurring historical gap (BLK 2015-2021) files `Revenues` +
  `RevenueFromContractWithCustomerExcludingAssessedTax` — **both already
  mapped.** So even that residue is NOT a tag-mapping gap; it's a
  dedup/anchor artifact, out of Risk 55's scope.

**Root cause of the silent heal.** Two fixes shipped for other reasons
closed Risk 55 as a side effect: (1) the `InterestAndDividendIncomeOperating`
revenue alias add (banks); (2) Risk 58's re-anchor of mart `fiscal_year`
to `year(period_end_date)` (Phase 5 session 3), which recovered the
52/53-week filer + comparative-year rows that the old `fy`-attribute
filter was dropping. Every bank that was missing in the Risk 55 banking
(GS, MS, WFC, JPM, C, BAC, PNC, SPGI) now reports FY2025 revenue.

**Decision (senior-DE, build-mode).** Do NOT run the speculative 8-file
seed/Jinja expansion + full cascade + Power BI re-import on closeout day.
The data proves it would not move FY2022-2025 (already 100%) and the only
incremental cells it could fill are a handful of 2015-2017 utility years
(`RevenueFromContractWithCustomerIncludingAssessedTax` /
`RegulatedAndUnregulatedOperatingRevenue`) immaterial to every headline
the dashboard shows. A rebuild + re-import carries more regression risk
than value. **Verify proved the work wasn't needed** — the highest-value
outcome a forward-verify pass can produce.

**Shipped instead.** `sql/verify/19_phase6_revenue_coverage_audit.sql` —
a permanent regression guard (the carry-forward the Risk 55 entry itself
proposed): per-canonical coverage on the latest complete FY + the two
prior complete FYs must hold >= 95% of the universe, plus a $9.5T revenue
reconciliation floor; FAILs at the SQL layer if a future mapping or
roster regression silently reopens the gap before it can surface as a BI
surprise. Bar + universe size derived from sp100_company_sector at
runtime so the checks self-adjust to roster changes.

**Carry-forward.** When a Risk is banked with a quantified impact, the
FIRST step of the dedicated fix session is to re-measure that exact
metric — fixes shipped for adjacent reasons routinely heal it early.
"Verify before you build" applies to closing Risks, not just opening
phases. Ship the regression guard regardless, so the closed Risk can't
silently reopen.

#### CI/CD — keyless GitHub OIDC deploy + end-to-end orchestrator gate.

**Forward-verify (current docs, not training).** Confirmed live action
majors: `actions/checkout@v6`, `actions/setup-python@v6`,
`aws-actions/configure-aws-credentials@v6`. OIDC pattern: `permissions:
id-token: write` + `role-to-assume` with no static keys signals the
action to exchange the GitHub OIDC JWT for temporary AWS credentials.

**Decision.** Keyless OIDC over static GitHub secrets — no long-lived AWS
keys stored in GitHub; the deploy role is assumed per-run. The portfolio
story is "keyless CI/CD to AWS."

**Surfaced code change (the forward-verify earning its keep).** The two
deploy scripts built explicit-key boto3 clients from `.env` — a path that
cannot carry the OIDC **session token**, so it would have failed in CI.
Unified both (`sync_phase3_artifacts_to_s3.py`,
`deploy_state_machine.py`) onto the boto3 default credential chain:
`load_dotenv()` exports `.env` keys into the env locally; OIDC exports
temporary creds (incl. session token) the same way in CI. One code path,
both environments.

**Wiring.** `.github/workflows/deploy.yml` runs on push to main touching
dbt/scripts/stepfunctions: checkout -> setup-python -> install boto3 ->
OIDC assume-role -> deploy dbt+wrapper to S3 -> deploy state machine ->
new `scripts/run_orchestrator.py` starts ONE orchestrator execution (Glue
dbt build -> 14-branch Athena verify) and polls to terminal, exiting
non-zero unless SUCCEEDED. That makes "dbt -> verify runs clean" a literal
CI gate. Bronze extraction stays OUT of CI by design (frozen for demo
durability, Risk 7) — CI transforms + verifies the existing raw zone.

**One-time AWS setup (Phil, documented in ORCHESTRATION_PIPELINE.md §4).**
IAM GitHub OIDC provider + `github-actions-lakehouse-deploy` role (trust
scoped to repo + refs/heads/main; permissions: s3:PutObject on the
lakehouse bucket, states:UpdateStateMachine / StartExecution /
DescribeExecution on the orchestrator) + repo variable
`AWS_DEPLOY_ROLE_ARN`.

**Carry-forward.** Any auth-mode switch (static keys -> OIDC) is also a
credential-resolution change in every script the CI invokes. Audit each
deploy script's boto3 client construction at the same time as writing the
workflow — the explicit-key client is the silent breakage that only
surfaces on the first CI run.

---

## PROJECT #3 — COMPLETE (2026-06-05)

Phase 6 session 2 closes the build. Bronze (S3 raw SEC EDGAR) ->
intermediate + Data Vault 2.0 warehouse (Glue/Athena/Iceberg) ->
canonical-collapsed Gold marts -> 6-page Power BI executive suite ->
Step Functions orchestration -> keyless GitHub OIDC CI/CD with an
end-to-end dbt-build-plus-verify gate. 62 Risks banked across the
