...
**Decision deferred to Phase 4 session 2.** NOT a mart bug — the collapse decision is upstream at sat_concept_value. Three resolution options at Phase 4 session 2 design pass: (a) switch MIN(value) → MAX(value) at sat_concept_value (less conservative, full-revenue bias; aligns with companies' public revenue announcements which typically quote the higher gross figure); (b) add per-canonical preferred-tag mapping (e.g., for 'revenue', prefer `Revenues` over `RevenueFromContractWithCustomerExcludingAssessedTax` when both exist — preserves historical comparability across pre/post-ASC 606 reporting); (c) accept the bias and document for analyst consumers (cheapest but worst analyst experience). Choice affects rebuild of sat_concept_value + all four Phase 4 marts (mart_pl_trend, mart_peer_benchmark, mart_financial_health, mart_growth_forecast).
