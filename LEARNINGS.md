...
**Carry-forward.** When a project-wide mitigation lands as a "standing cascade command" (here: `--threads 2`), every layer that invokes the same tool must apply the same flag. Local dev / CI / production / Step Functions / Glue / Airflow / Dagster — each is a separate invocation path. Pattern: encode the flag as a project config (`dbt_project.yml threads:` or seed) rather than as a per-command CLI argument so all layers inherit by default. For this project specifically, follow-up Phase 6 polish: move `threads: 2` into the `dbt_project.yml` `+threads` config on the marts layer so the flag is automatic regardless of invocation path.
