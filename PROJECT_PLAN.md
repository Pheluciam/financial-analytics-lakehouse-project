...
| **Phase 4** | Gold marts + forecasting | 4 mart models in dbt (mart_pl_trend, mart_peer_benchmark, mart_financial_health, mart_growth_forecast), `scripts/forecast.py` using statsmodels (Holt-Winters / ARIMA) producing Parquet for `mart_growth_forecast` per Risk 38 (Prophet rejected on annual cadence + Stan compile footprint), mart-shape smoke test against Power BI for EACH mart at creation time (Project #2 carry-forward — catches mart-architecture problems early), `GOLD_MARTS_PIPELINE.md`, `DBT_PIPELINE.md` (Gold section). Risks 38-39 baked in at design time at session 14 forward-verify pass. |
