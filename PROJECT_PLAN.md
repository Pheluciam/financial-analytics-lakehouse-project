...
| **Phase 3** | Step Functions orchestration | `stepfunctions/state_machine.json`, IAM execution role, on-demand trigger (no schedule per demo-durability principle 2), `ORCHESTRATION_PIPELINE.md`. **dbt-runtime LOCKED as Glue Python Shell** at the Phase 3 kickoff forward-verify pass (session 11 close, 2026-05-29). Senior-DE default per Risk 3: 480-min timeout (vs Lambda's 15-min hard cap per Risk 28), pyathena 2.5.3 pre-installed in analytics library set narrowing the dep-install delta to dbt-core + dbt-athena, no container build / no ECR, no VPC required, lowest IAM expansion, Free Tier fit with ~8x margin at daily-run cadence. Lambda Container Image rejected on 15-min cap + container build + ECR overhead; ECS Fargate rejected on overkill IAM + VPC + cost for daily-cadence orchestration. Step Functions state machine pattern: ONE Glue Python Shell task running `dbtRunner().invoke(["build"])` (one process per Risk 24 + exit-code-only success detection per Risk 25) + complementary Athena `.sync` tasks for raw SQL verify queries per Risk 29 (Step Functions native Athena integration runs raw SQL, not dbt orchestration). Risks 24-29 in force at design time; Risk 26 (Glue Python Shell 3.6 EOL 2026-03-01) pins Python 3.9; Risk 27 (cold-start dep-install timing) measured on first job run. |
