...
- **JSON authoring + structure (added 2026-05-29, Phase 3 session 14 close).** Hand-authoring + reading JSON for cloud-tool configuration: AWS Step Functions ASL JSON state machines, AWS IAM policy / trust policy JSON, AWS Glue job config JSON, Azure ARM templates / ADF pipeline JSON, package.json + tsconfig.json shape recognition. Same schema-recognition mental map principle as YAML — picking up the shape, the required vs optional keys, where common mistakes happen (trailing commas not allowed; comments not allowed; escaping rules for embedded strings / quotes / backslashes). Phil's framing 2026-05-29: "SQL + dbt + YAML + JSON + env-vars + a bit of Python" is the realistic credibility surface for next-step roles; JSON authoring fluency directly maps to AWS / Azure config surfaces he'll encounter daily in DE / DA pipeline work. Triggers for new JSON formats encountered: open the AWS / vendor schema reference, identify the top-level keys, work an example, hand-author one from scratch without copy-paste.
...
**Suggested 8-week outline — INTERLEAVED / PARALLEL shape (refreshed 2026-05-29 at Phil's Phase 3 session 14 ask).**

**Why interleaved, not sequential.** The earlier 2026-05-28 outline was depth-first sequential: week 1 = SQL only, week 4 = PBI only, week 7 = YAML / JSON / Git only. Phil flagged the real risk at Phase 3 session 14 close: if he's actively job-hunting throughout the journey (and the locked rule is "if Phil lands a role partway through, STOP the journey") then a sequential shape leaves him gap-exposed to whole categories of role until he reaches that week. A perfect Senior BI Developer JD that mentions JSON in week 3 is wasted if Phil hasn't seen JSON yet. The fix is interleaved: every week touches every domain at maintenance density; the week's emphasis domain gets the depth slot. By end of week 1 Phil has seen SQL deep + one rep each of dbt + PBI/DAX + YAML + JSON + Python + Git. By end of week 4 he's authored small artifacts in every domain. By end of week 8 the depths are layered AND the breadth is established.

**Standing weekly shape (every week, all 8 weeks).** Two ~2-hour sessions per week (matches Phase 0 cadence target). Each session split across:

- **~40% emphasis domain (the week's depth slot).** Deep work — refactor / author / tune. Heaviest single block.
- **~30% maintenance slots across the other domains.** Each maintenance slot is 15-25 min: read a small artifact, run a small drill, do a quiz prompt. Domains rotate so every domain gets touched at least once per week, ideally twice.
- **~15% interview-prep micro-drill.** One project-narration rep (one of the 8 portfolio repos in 90 seconds), OR one behavioral STAR answer from LEARNINGS material, OR one system-design lite sketch. Rotates across weeks.
- **~15% quiz warm-up.** 5-8 quiz questions at the start of the session. Domain mix calibrated to weak areas tracked in the quiz-log file.

**Complexity gradient ACROSS all domains in parallel (refinement 2026-05-29).** Within every domain — maintenance slots AND emphasis weeks — the difficulty ramps WITH the calendar, not WITH the depth schedule. Weeks 1-2 maintenance slots are "read this file, name what it does, identify the top-level keys"; weeks 3-4 are "modify this file safely, predict what the change will do"; weeks 5-6 are "author a small one from scratch in a dummy scenario, debug your own mistakes"; weeks 7-8 are "author a non-trivial one, defend every choice, explain the trade-offs". By week 8 Phil is at solid-beginner-to-early-intermediate across every domain in parallel, not expert-in-one-domain + zero-in-others. The repetition-with-gradient pattern is how skills consolidate in memory — Phil's framing 2026-05-29: "I'm learning a little bit and getting a little bit harder week by week of everything, then it'll consolidate more in my mind." Beats one-and-done depth sprints that fade if not revisited.

**Emphasis rotation (the depth schedule).** Same eight topics as before, just decoupled from breadth coverage:

- **Week 1 — SQL + T-SQL deep dive (emphasis).** Window functions, complex CTEs, dialect translation, T-SQL stored procedures + MERGE + DECLARE. Maintenance touches on dbt (read 1 model + run dbt build on a project), PBI (open a .pbix + write 1 small measure), YAML (read 1 _models.yml), JSON (read 1 Step Functions ASL OR 1 IAM policy from Project #3), Python (read 1 script + 1 traceback drill), Git (1 branch + merge rep).
- **Week 2 — dbt patterns part 1 (emphasis).** Materialization strategy, test design, layering philosophy. Phil extends Project #3 with one new custom singular test + one materialization tuning. Maintenance on SQL, PBI, YAML, JSON, Python, Git per the standing shape.
- **Week 3 — dbt patterns part 2 (emphasis).** Jinja macros, dbt-utils + dbt-expectations, custom generic tests, performance optimization. Phil writes one new dbt macro from scratch. Maintenance on SQL, PBI, YAML, JSON (hand-author 1 small IAM policy from scratch in a dummy scenario), Python, Git.
- **Week 4 — Power BI + DAX depth (emphasis).** DAX patterns, semantic-layer modeling, performance tuning. Phil extends Project #2 or #3 .pbix with 2-3 new advanced measures + one performance-tuned page. Maintenance on SQL, dbt, YAML, JSON, Python, Git.
- **Week 5 — Python basics + scripting (emphasis).** pathlib, requests, basic pandas, venv + deps, structured logging, reading tracebacks. Phil writes one small Python utility from scratch. Maintenance on SQL, dbt, PBI, YAML, JSON (hand-author 1 small Step Functions ASL from scratch in a dummy scenario), Git.
- **Week 6 — Modeling + architecture patterns (emphasis).** Kimball walkthrough of Project #2 + DV2.0 reinforcement of Project #3 + medallion review. System-design lite practice. Maintenance on SQL, dbt, PBI, YAML, JSON, Python, Git.
- **Week 7 — Git + CI/CD + YAML / JSON / env / config fluency (emphasis).** Git deep dive (branching, rebase, conflict resolution, recovery). GitHub Actions YAML for dbt CI + ruff lint + sqlfluff. Pre-commit hooks. **YAML fluency across the stack**: dbt schema YAMLs, dbt_project.yml, profiles.yml, GitHub Actions workflow YAML, Docker Compose. **JSON fluency across the stack**: Step Functions ASL, IAM policy + trust policy, Glue job config, package.json + tsconfig.json shape, jq for command-line JSON triage. **Env file + secrets management**: .env conventions, python-dotenv pattern, .env.example, .gitignore secrets patterns, GitHub Actions secrets, AWS credential chain. **AI-tools fluency** consolidation. Maintenance still on SQL, dbt, PBI, Python — even on the consolidation week, the other domains stay warm.
- **Week 8 — Interview prep intensive (emphasis).** Mock interviews per target role (Senior DA / BI Developer / BI Engineer). Project-narration practice for all 8 repos. System-design lite drill. Behavioral STAR practice. Salary expectation framing. Maintenance slots still touch every technical domain so they don't atrophy between mock interviews.

**Domain-touch matrix (which week first deep-touches which thing).** Phil should be employable against most JDs by end of week 2-3, not end of week 8. The maintenance slots establish breadth FAST; the depth weeks layer on top.

| Domain | First TOUCH | First DEPTH | Established by |
|---|---|---|---|
| SQL / T-SQL | Week 1 | Week 1 | Week 1 |
| dbt | Week 1 (maintenance) | Week 2-3 | Week 3 |
| Power BI / DAX | Week 1 (maintenance) | Week 4 | Week 4 |
| Python | Week 1 (maintenance) | Week 5 | Week 5 |
| YAML | Week 1 (maintenance) | Week 7 | Week 7 |
| JSON | Week 1 (maintenance) | Week 7 | Week 7 |
| Git | Week 1 (maintenance) | Week 7 | Week 7 |
| Modeling / Architecture | Week 1 (maintenance — read existing) | Week 6 | Week 6 |
| Interview narration | Week 1 (1 micro-drill / week) | Week 8 | Week 8 |

**Compressible to 6 weeks (preserved from 2026-05-28).** Merge weeks 5+6 and weeks 7+8 if Phil lands a role partway through OR if he's accelerating. The interleaved shape compresses naturally — the standing weekly structure is the same, just fewer weeks.
...
- 2026-05-29 (later — Phil refinement) — **Complexity gradient layered ACROSS all domains in parallel.** Within every domain (maintenance slots AND emphasis weeks), difficulty ramps with calendar week, not depth schedule. Weeks 1-2 = read + identify. Weeks 3-4 = modify + predict. Weeks 5-6 = author small + debug own mistakes. Weeks 7-8 = author non-trivial + defend trade-offs. By week 8 Phil is solid-beginner-to-early-intermediate across every domain in parallel, not expert-in-one-domain + zero-elsewhere. Repetition-with-gradient is the consolidation pattern — beats one-and-done depth sprints that fade. Phil's framing: "I'm learning a little bit and getting a little bit harder week by week of everything, then it'll consolidate more in my mind."
- 2026-05-29 (later) — **Training journey restructured from sequential to interleaved/parallel shape.** Driver: Phil flagged the real risk at Phase 3 session 14 close — sequential 8-week outline (week 1 = SQL only, week 4 = PBI only, week 7 = YAML / JSON / Git only) leaves him gap-exposed to whole categories of role until he reaches the relevant week. The locked rule "if Phil lands a role partway through, STOP the journey" means a JSON-mentioning JD in week 3 is wasted if Phil hasn't touched JSON yet. Restructured to: every week touches every domain at maintenance density (15-25 min slots); the week's emphasis domain still gets the depth slot (~40% of session). By end of week 1 Phil has seen SQL deep + one rep each of dbt + PBI / DAX + YAML + JSON + Python + Git. Domain-touch matrix added showing first-touch + first-depth + established-by week per domain — Phil is employable against most JDs by end of week 2-3, not end of week 8. Standing weekly shape: ~40% emphasis + ~30% maintenance across other domains + ~15% interview-prep micro-drill + ~15% quiz warm-up. Eight-topic emphasis rotation preserved from 2026-05-28; compressibility to 6 weeks preserved.
- 2026-05-29 — **JSON authoring fluency added to training journey scope.** Driver: Phil's framing at Phase 3 session 14 close — "SQL + dbt + YAML + JSON + env-vars + a bit of Python" is the realistic next-step credibility surface for BI Developer / BI Engineer / Senior DA roles. JSON authoring directly maps to AWS / Azure config surfaces (Step Functions ASL, IAM policies, Glue job config, ADF pipeline JSON, ARM templates). Two changes landed in this file: (a) Code-focus section gets a new JSON authoring + structure bullet alongside the existing YAML bullet; (b) Week 7 of the 8-week outline retitled "Git + CI/CD + YAML / JSON / env / config fluency + AI-tools" with explicit JSON hand-author exercises (one Step Functions ASL from scratch + one IAM policy from scratch against a fresh dummy scenario, proves the schema lives in his head not just in the existing Project #3 files). JSON authoring traps explicit: trailing commas not allowed, comments not allowed, escaping quotes / backslashes / newlines in embedded strings.
