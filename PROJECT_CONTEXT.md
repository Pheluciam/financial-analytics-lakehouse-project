...
| Field | Value |
|---|---|
| Active phase | **Phase 5 session 1 v1 SHIPPED + complete 5-page redesign queued 2026-05-31.** Phase 5 session 1 shipped a working v1 of the executive overview page (`powerbi/financial_analytics.pbix`) — data model (4 marts + dim_company sourced from sp100_company_sector seed + dim_as_of_date sourced from DISTINCT mart_pl_trend snapshot dates, all imported via the locked ODBC SELECT-per-table pattern, 8 active many-to-one relationships on cik + as_of_date), hidden `_Measures` table with the locked architectural discipline (placeholder Column1 hidden in report view, all measures live on _Measures not on fact tables), 4 KPI cards (Total Revenue (Latest FY) + Revenue YoY % + Total Net Income (Latest FY) + Net Margin (Latest FY) — all using the locked self-correcting Latest-Complete-FY pattern with ≥80-of-107 company-count threshold, all formatted 1-decimal precision via measure-level format string overrides), hero revenue trajectory line chart FY2009-2024 historical-only sourced from mart_pl_trend (forecast viz pulled out of Page 1 scope and deferred to dedicated Page 5 Growth/Forecast per the locked clean-page-per-mart pattern), 2 slicers at top-right (gics_sector + as_of_date, Original Slicer with Dropdown style), Executive built-in theme, footer caveat strip carrying source + universe + FY2024 coverage + snapshot metadata in Bloomberg/FactSet-style pipe-separated format. Athena audit at session midpoint shipped 4 diagnostic SQLs (canonical_concept distribution + company count per fiscal_year + revenue sum per fiscal_year + S&P 100 universe missing FY2024 revenue) surfacing the Risk 55 sector-specific tag mapping gap (97/107 FY2024 coverage, 10-12% under-count concentrated in Financials). **Phil's design call at session close** — v1 looks too generic for a Project #3 portfolio piece (reads like every beginner Power BI tutorial pattern, no analytical distinction from Projects #1 and #2); complete redesign across all 5 pages queued for sessions 2-6 per the spec landed in `POWERBI_PIPELINE.md` section 3 covering: Page 1 Executive Overview redesign (KPI cards with embedded sparkline backgrounds + annotated trajectory chart with COVID/ASC 606/recession reference overlays + sector treemap + top movers strip + drill-through to Page 6 Company Detail), Page 2 P&L Trend deep-dive (dual-axis revenue + net income chart + sector-mix stacked area + 11-sector small multiples + margin trend heatmap), Page 3 Peer Benchmarking (sector-driven bubble chart encoding revenue × net income × assets × peer_rank + sector benchmark gauges + top/bottom 10 ranked bars + custom tooltip page), Page 4 Financial Health (Decomposition Tree from total revenue → sector → company + 8-ratio gauge grid with sector/S&P 100 traffic-light comparison + 10-year ratio trajectory + health heatmap), Page 5 Growth/Forecast (combined historical + forecast trajectory with 95% CI bands + 11-sector forecast small multiples + top forecasted growth ranking + per-company model metadata panel), Page 6 Company Detail (drill-through target from any of Pages 1-5). 3 new Risks banked at session close in LEARNINGS.md: Risk 55 (sector-specific us-gaap revenue tag mapping gap), Risk 56 (forecast horizon varies per company creating apparent cliff at rightmost forecast year in multi-company aggregations), Risk 57 (PBI authoring discipline — ship a deliberate design BEFORE clicking, not iteratively patch through the visual; locked carry-forward for sessions 2-6: each session opens with a 1-page design call before any PBI clicks). v1 .pbix committed as the working baseline; sessions 2-6 extend the same `powerbi/financial_analytics.pbix` per the locked continuous-publish convention (no separate versioned filenames; git is the version control). |
| Next phase | **Phase 5 session 2 — Executive Overview page REDESIGN** per `POWERBI_PIPELINE.md` section 3.1. Per the locked Risk 57 carry-forward rule: open the session with a 1-page design call (what the page demonstrates + what visuals encode what + why this is portfolio-distinctive vs Projects #1 and #2) BEFORE any PBI clicks; bank the design call output as the session's pre-implementation spec; implementation iterates against the spec, not against the rendered output. Deliverable target: KPI cards with embedded sparkline backgrounds (per Microsoft Learn 2026 sparkline-in-card pattern) + annotated revenue trajectory chart (COVID + ASC 606 + 2008-2009 recession reference overlays) + sector treemap on the right + top movers strip on the bottom + drill-through to a new Page 6 (Company Detail). Demonstrates the Data Vault → Gold marts architecture in the visual layer. |
| Last session closed | 2026-05-31 (Phase 5 session 1 — v1 SHIPPED + complete 5-page redesign spec landed in POWERBI_PIPELINE.md sections 1-6 + Risks 55-57 banked in LEARNINGS.md + standing PBI conventions locked in POWERBI_PIPELINE.md section 2) |
| Last bundled commit | 2026-05-31 — Phase 5 session 1 bundle (pending push at session close) |
