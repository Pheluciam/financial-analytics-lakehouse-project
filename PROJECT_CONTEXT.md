...
| Field | Value |
|---|---|
| Active phase | **Phase 2 in progress.** Session 9 CLOSED 2026-05-29 — first multi-active satellite (MAS) in the project, the audit lineage layer. sat_concept_canonical shipped: parent = hub_concept (5 canonicals), 8 active rows (4 revenue alias raw XBRL tags mapping to canonical 'revenue' + 4 identity-mapped tags), CDK = SHA-256 of raw concept_name (stable source-provided type code per Scalefree priority rule), degenerate payload (CDK == payload, hashdiff structurally constant per (parent, CDK) by construction), MAS-adapted SCD-2 anti-join partitions by (parent_hk, sub_sequence_key). Defends sat_concept_value's MIN(value) information-loss decision (session 8) by preserving regulatory-defensible provenance to the original XBRL US-GAAP tag every fact was reported under. First MAS in the project — new DV2.0 mechanic relative to the 1:1 sat shape of sessions 6/7/8. Two new Risks banked at the forward-verify pass BEFORE code shipped: Risk 17 (degenerate MAS payload — CDK == payload; name the degeneracy in model body, keep hashdiff column for visual consistency + future-proofing), Risk 18 (CDK selection priority — stable source-provided type code over sub-sequence auto-numbering per Scalefree explicit guidance; raw XBRL tag is a stable taxonomy identifier so direct-hash beats auto-number). Forward-verify cardinality probe predicted 8 rows exactly; dbt run #1 delivered OK 8 in 11.33s first try. 11/11 dbt schema tests PASS (including 3-column MAS composite PK via dbt_utils.unique_combination_of_columns). 14/14 SQL structural verify PASS in 1.84 sec (2 more than standard sat verify because MAS carries sub_sequence_key hash column + MAS-specific cardinality invariant guard). Idempotency proven via second dbt run [OK 0 in 25.72s] — MAS NOT EXISTS anti-join filtered all 8 inbound rows. 10/10 ENGINEERING_STANDARDS audit PASS. Cumulative warehouse-layer: 88/88 dbt schema + 90/90 SQL structural across 3 hubs + 2 links + 4 sats. |
| Next phase | Phase 2 has 2 remaining sessions locked: **session 10 — Business Vault PIT + Bridge tables** (Scalefree-canonical Business Vault layer — PIT for per-as-of-date hub+sat snapshots, Bridge for pre-computed hub-link-hub join graphs at a point in time; demonstrates full DV2.0 fluency beyond the Raw Vault, materially accelerates Phase 4 mart queries by collapsing multi-join walks to straight SELECTs). **Session 11 — Phase 2 close** (phase-boundary structural audit per ENGINEERING_STANDARDS + LEARNINGS Phase 2 reflection + README status-line refresh + Phase 3 forward-verify pass + dbt-runtime choice between Glue Python Shell vs Lambda Container Image per Risk 3). Est. 60-120 min for session 10, 45-90 min for session 11. |
| Last session closed | 2026-05-29 (Phase 2 session 9 — CLOSE) |
| Last bundled commit | 2026-05-29 — Phase 2 session 9 bundle (sat_concept_canonical.sql as the project's first multi-active satellite — MAS pattern with stable-type-code CDK + degenerate payload + MAS-adapted SCD-2 anti-join filter partitioning by (parent_hk, sub_sequence_key); warehouse/_models.yml extended with sat_concept_canonical block = 11 new schema tests including 3-column MAS composite PK; sql/verify/10 = 14 structural checks including MAS-specific cardinality invariant (8 distinct parent×CDK pairs) + parent coverage (5 canonicals) + 3-column composite PK uniqueness; DBT_PIPELINE.md sections 8.20 + 8.21 covering MAS pattern walkthrough + Risks 17/18 framing + verification surface + cumulative 88/88 + 90/90 stats; GLOSSARY.md section 2 extended with Multi-Active Satellite + Sub-sequence key / CDK entries; LEARNINGS.md Risks 17 + 18 banked at the forward-verify pass BEFORE code shipped) |
