...
| Field | Value |
|---|---|
| Active phase | **Phase 2 in progress.** Session 10 CLOSED 2026-05-29 — first Business Vault objects shipped, Scalefree-canonical layer between Raw Vault and Phase 4 marts. Three models: dim_as_of_dates (10-row fiscal year-end spine, 2016-12-31 through 2025-12-31, business_vault as-of-dates input to both PIT and Bridge), pit_link_filing_concept_period (single-sat PIT on the most-queried link spine + sat_concept_value, 634,431 rows), bridge_company_concept_period (5-hop hub-link-hub Bridge spanning hub_company → link_company_filing → hub_filing → link_filing_concept_period → hub_concept, 634,431 rows — identical to PIT by construction since both share the same link × as_of_date × filed_date visibility filter). Both PIT and Bridge use filed_date (from sat_filing_metadata) as the temporal anchor instead of canonical load_datetime — project's load_datetime captures ingestion-time (dbt-run wall clock, all rows stamped May 2026), not observation-time (Risk 23 deviation, documented and justified). Materialized as plain Iceberg tables (not incremental merge) per business_vault layer defaults in dbt_project.yml — query helpers rebuilt each refresh, structurally avoids the Risk 2 Iceberg-merge bug class. Five new Risks banked at the kickoff forward-verify pass BEFORE code shipped: Risk 19 (PIT pattern needs 2+ sats per parent to materialize value; ship ONE demonstrative PIT on most-queried parent, frame single-sat honestly), Risk 20 (Bridge eff_sat metadata optional; insert-only-current links use simpler shape without end-date columns), Risk 21 (as-of-dates cardinality multiplies BV row counts; picked fiscal-year-end yearly over quarterly to balance demonstrative multi-snapshot mechanic against storage budget), Risk 22 (ghost-record pattern deferred indefinitely; LEFT JOIN + NULL substitutes), Risk 23 (load_datetime ingestion-time vs observation-time semantic mismatch; PIT/Bridge anchor on filed_date for meaningful temporal snapshots across the 10-year demonstrative horizon). Forward-verify cardinality predictions: ~500-700K rows each → actual 634,431 both, within prediction band. dbt run #1 delivered 3 OK with PIT 634,431 in 29.96s + Bridge 634,431 in 30.27s. 33/33 dbt schema tests PASS (6 dim + 9 PIT + 18 Bridge incl. 2 composite-PK tests in 39.34s). 11/11 SQL structural verify PASS for PIT in 3.85 sec; 13/13 PASS for Bridge in 7.96 sec. Idempotency proven via third dbt run — 3 OK rebuilds with identical 634,431 row counts on both BV tables (table materialization is the canonical BV pattern; full rebuild + deterministic hash chain + deterministic JOIN + deterministic WHERE filter = byte-identical content). 10/10 ENGINEERING_STANDARDS audit PASS (third consecutive session running the audit as an explicit numbered task — sessions 8/9/10 unbroken after the 5/6/7 miss streak). Cumulative warehouse + business-vault layer: 121/121 dbt schema + 114/114 SQL structural across 3 hubs + 2 links + 4 sats + 1 dim + 1 PIT + 1 Bridge. |
| Next phase | Phase 2 has 1 remaining session locked: **session 11 — Phase 2 close** (phase-boundary structural audit per ENGINEERING_STANDARDS + LEARNINGS Phase 2 reflection + README status-line refresh + Phase 3 forward-verify pass + dbt-runtime choice between Glue Python Shell vs Lambda Container Image per Risk 3). Est. 45-90 min. |
| Last session closed | 2026-05-29 (Phase 2 session 10 — CLOSE) |
| Last bundled commit | 2026-05-29 — Phase 2 session 10 bundle (first Business Vault objects: dim_as_of_dates.sql + pit_link_filing_concept_period.sql + bridge_company_concept_period.sql as the first Scalefree-canonical PIT + Bridge query-helpers in the project; business_vault/_models.yml = 33 new schema tests incl. 2 composite-PK tests + 5 FK relationships closures on Bridge; sql/verify/11 = 11 PIT structural checks; sql/verify/12 = 13 Bridge structural checks; dbt_project.yml extended with business_vault layer block (table + iceberg + parquet); DBT_PIPELINE.md sections 8.22-8.25 covering BV overview + PIT walkthrough + Bridge walkthrough + verification surface + cumulative 121/121 + 114/114 stats; GLOSSARY.md section 2 extended with Business Vault + Point-in-Time table + Bridge table + As-of-date table entries (4 new); LEARNINGS.md Risks 19/20/21/22/23 banked at the forward-verify pass BEFORE code shipped; PROJECT_PLAN.md section 7 + Phase 2 status table updated to mark Business Vault as shipped) |
